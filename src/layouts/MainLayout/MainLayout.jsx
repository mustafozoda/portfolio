...
const SortableTab = ({ tab, activeTab, setActiveTab, closeTab, navigate }) => {
  ...
  return (
    <div
      ...
      className={`flex justify-center border-border min-w-12 items-center gap-2 text-xs px-3 py-1 border rounded-[8px] cursor-pointer ${
        activeTab === tab.path
...
const MainLayout = () => {
  ...
  return (
    <div className="w-full container h-screen bg-dark2 flex flex-col">
        ...
        <div className="right-side-box-for-outlet bg-dark2 flex-col flex justify-center items-center flex-grow h-full">
          <div className="nav-path flex-col w-full h-[8%] flex text-xs text-gray-400 font-mono">
            ...
            >
              ...
              >
                <div className="h-[60%] border-border border-b px-4 flex items-center gap-1 overflow-x-auto">
                  {tabs.map((tab) => (
                    <SortableTab
                      key={tab.path}
                      tab={tab}
      ...
      <div className="w-full bg-dark h-[3%]">
        <Footer />
      </div>
    </div>
  );
